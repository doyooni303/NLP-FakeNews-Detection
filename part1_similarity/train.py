...
def training(model, num_training_steps: int, trainloader, validloader, criterion, optimizer, scheduler,
    ...
    train_mode = True
    while train_mode:
        for doc, targets, cat, length in trainloader:
            # batch
            doc, targets, cat, length = convert_device(doc, device), targets.to(device), cat.to(device), length.to(device)
            ...
            if opt_cond:
                ...
                if ((step+1) // accumulation_steps) % log_interval == 0 or step == 0: 
                    _logger.info('TRAIN [{:>4d}/{}] Loss: {loss.val:>6.4f} ({loss.avg:>6.4f}) '
                                ...
                                rate       = doc['input_ids'].size(0) / batch_time_m.val,
                                rate_avg   = doc['input_ids'].size(0) / batch_time_m.avg,
...
def evaluate(model, dataloader, criterion, log_interval: int, device: str = 'cpu', sample_check: bool = False):
    ...
    model.eval()
    with torch.no_grad():
        for idx, (doc, targets, cat, length) in enumerate(dataloader):
            doc, targets, cat, length = convert_device(doc, device), targets.to(device), cat.to(device), length.to(device)
