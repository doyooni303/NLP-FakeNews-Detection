...
class BERT_categoryDataset(FakeDataset):
    def __init__(self, tokenizer, max_word_len: int, name: str,
                 use_cat: bool = True):
        super(BERT_categoryDataset, self).__init__(tokenizer=tokenizer, use_cat = use_cat, name = name)
    ...
    def transform(self, title: str, text: list, category: str = '', subcategory: str = '') -> dict:
        sent_list = [category + subcategory + title] + text

        if 'klue' in self.name:
            src = [self.tokenizer.tokenize(d_i) for d_i in sent_list]
        else:
