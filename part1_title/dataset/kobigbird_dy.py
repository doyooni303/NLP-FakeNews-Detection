...
class KoBigBirdDataset(FakeDataset):
    def __init__(self, tokenizer, max_word_len: int):
        super(KoBigBirdDataset, self).__init__(tokenizer=tokenizer)
    ...
    def transform(self, title: str, text: list) -> dict:
        ...
        src = self.length_processing(src, "main")
    ...
    def tokenize(self, src: list) -> List[torch.Tensor]:
        src_subtokens = [[self.vocab.cls_token] + src[0] + [self.vocab.sep_token]] + [
            sum(src[1:], []) + [self.vocab.sep_token]
        ]
    ...
    def length_processing(self, src: list, input_type: str) -> list:
        ...
        max_word_len = self.max_word_len - 3
