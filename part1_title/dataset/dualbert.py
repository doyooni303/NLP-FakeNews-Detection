...
class DualBERTDataset(FakeDataset):
    def __init__(self, tokenizer, max_word_len: int, max_category_len: int, name: str,
                 use_cat: bool = False):
        super(DualBERTDataset, self).__init__(tokenizer=tokenizer, use_cat = use_cat, name = name)
        ...
        try:
            self.name = tokenizer.name_or_path
        except:
            self.name = 'kobert'
        
        if 'klue' in self.name:
            self.CLS = '[CLS]'
            self.PAD = '[PAD]'
            self.SEP = '[SEP]'

            self.cls_idx = self.vocab[self.CLS]
            self.pad_idx = self.vocab[self.PAD]
            self.sep_idx = self.vocab[self.SEP]            
    ...
    def transform(self, title: str, text: list, category:str, subcategory:str) -> dict:
        ...
        if 'klue' in self.name:
            main_src = [self.tokenizer.tokenize(d_i) for d_i in main_sent_list]
            category_src = [self.tokenizer.tokenize(d_i) for d_i in category_sent_list]
        else:
    ...
    def tokenize(self, src: list) -> List[torch.Tensor]:
        ...
        if 'klue' in self.name:
            src_subtokens = [[self.CLS] + src[0] + [self.SEP]] + [sum(src[1:],[]) + [self.SEP]]
        else:
