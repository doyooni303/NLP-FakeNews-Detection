...
def convert_device(inputs: dict, device: str) -> dict:
    for key in inputs.keys():
        if type(inputs[key]) == dict:
            for k in inputs[key].keys():
                inputs[key][k] = inputs[key][k].to(device)
        else:
...
def extract_wrong_ratio(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Category"] = df["filename"].apply(
        lambda x: os.path.basename(os.path.abspath(os.path.join(x, "../../")))
    )
    ...
    cnt_df["wrong / total (%)"] = cnt_df.apply(
        lambda x: f"{x.wrong_cnt} / {x.total_cnt} ({x.wrong_cnt/x.total_cnt:.2%})",
        axis=1,
    )
...
def select_wrong_case_topN(df: pd.DataFrame, cat: str, n: int):
    assert cat in [
        ...
        "Clickbait_Auto",
        "NonClickbait_Auto",
    ], "cat should be either 'Clickbait_Direct','Clickbait_Auto','NonClickbait_Auto'"
    # define wrong pred
    ...
    wrong_case = (
        pd.concat(
            [
                ...
                df_incorrect[df_incorrect.category == cat]["outputs"].apply(
                    lambda x: eval(x)[pred]
                ),
            ],
            axis=1,
        )
